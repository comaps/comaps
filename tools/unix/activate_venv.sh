...
# protobuf must be the last "3.x" line on PyPI: it ships a pure-Python wheel that
# installs on any Python version, the checked-in *_pb2.py files load under it, and
# it matches the range enforced in CMakeLists.txt (>=3.20, <4.0). Note Google's
# Python package jumped straight from 3.20 to 4.21 - there is no 3.21.x on PyPI
# (distro packages like Ubuntu's python3-protobuf "3.21.x" map to PyPI 4.21.x).
