...
{
  if (segments.empty() || featureType == Classificator::INVALID_TYPE)
    return;

  double constexpr kSearchRadiusM = 10.0; //radius (in meters)
...
void RoutingManager::CollectTrafficLights(vector<RouteSegment> const & segments, m2::PointD const & startPt, vector<m2::PointD> & trafficLights)
{
...
void RoutingManager::CreateTrafficLightMarks(vector<m2::PointD> && trafficLights)
{
  ...
  {
    auto es = m_bmManager->GetEditSession();
    for (auto const & pt : trafficLights)
      es.CreateUserMark<TrafficLightMark>(pt);
  });
}
...
bool RoutingManager::InsertRoute(Route const & route)
{
  ...
  RoadWarningsCollection roadWarnings;
  vector<m2::PointD> trafficLights;
