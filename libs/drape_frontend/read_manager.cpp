                                         ...
                                         ref_ptr<MetalineManager> metalineMng)
{
  ...
  auto context = make_unique_dp<EngineContext>(TileKey(tileKey, m_generationCounter, m_userMarksGenerationCounter),
                                               ...
                                               m_isolinesEnabled, m_indoor, m_mapLangIndex);
...
void ReadManager::SetIndoor(indoor::Active const & indoor)
{
  if (m_indoor.m_level != indoor.m_level || m_indoor.m_complex != indoor.m_complex)
  {
    m_modeChanged = true;
    m_indoor = indoor;
  }
