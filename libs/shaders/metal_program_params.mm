...
{
template <typename T>
void ApplyBytes(ref_ptr<dp::GraphicsContext> context, ref_ptr<dp::GpuProgram> program, T const &params)
{
  ...
  if (vsBindingIndex >= 0)
    [encoder setVertexBytes:(void const *)&params length:sizeof(params) atIndex:vsBindingIndex];

  auto const fsBindingIndex = p->GetFragmentShaderUniformsBindingIndex();
  if (fsBindingIndex >= 0)
    [encoder setFragmentBytes:(void const *)&params length:sizeof(params) atIndex:fsBindingIndex];
}
}  // namespace
