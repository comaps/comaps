...
std::string DebugPrint(SystemProgram p)
{
  ...
  case SystemProgram::SystemProgramsCount: CHECK(false, ("Try to output SystemProgramsCount"));
  }
                                                ...
                                                std::map<uint8_t, uint8_t> const &layout)
{
  ...
  return make_unique_dp<dp::metal::MetalGpuProgram>(programName, vertexShader, fragmentShader, vsUniformsBindingIndex,
                                                    fsUniformsBindingIndex, std::move(vsTextureBindingInfo),
                                                    std::move(fsTextureBindingInfo), vertexDesc);
}
