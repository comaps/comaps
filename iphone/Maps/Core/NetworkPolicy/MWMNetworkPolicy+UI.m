...
- (void)callOnlineApi:(MWMBoolBlock)onlineCall forceAskPermission:(BOOL)askPermission
{
  ...
  {
  case MWMConnectionTypeNone: onlineCall(NO); break;
  case MWMConnectionTypeWifi: onlineCall(YES); break;
  case MWMConnectionTypeCellular:
    switch (self.permission)
    {
    case MWMNetworkPolicyPermissionAsk: [self askPermissionWithCompletion:onlineCall]; break;
    case MWMNetworkPolicyPermissionAlways: onlineCall(YES); break;
