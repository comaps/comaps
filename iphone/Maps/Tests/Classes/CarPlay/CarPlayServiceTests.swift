...
final class CarPlayServiceTests: XCTestCase {
  ...
  func testCarPlayManeuverSymbolUsesBlackAndWhiteVariants() throws {
    let symbol = try XCTUnwrap(CarPlayManeuverSymbol.image(named: "ic_cp_simple_left"))
  ...
  func testNumberedRoundaboutPreservesBlackAndWhiteVariants() throws {
    let plain = try XCTUnwrap(CarPlayManeuverSymbol.image(named: "ic_cp_round"))
    let numbered = try XCTUnwrap(CarPlayManeuverSymbol.image(named: "ic_cp_round", exitNumber: 3))
  ...
  func testCarPlayLaneImageSetUsesWhiteLightContentAndBlackDarkContent() throws {
    ...
    let imageSet = try XCTUnwrap(CarPlayLaneSymbol.imageSet(for: lanes))
